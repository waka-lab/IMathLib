#ifndef IMATHLIB_INTERFACE_MODEL_CONTROL_HPP
#define IMATHLIB_INTERFACE_MODEL_CONTROL_HPP
...
namespace iml {
	namespace i_f {
		...
		// 頂点配列データ(頂点配列,サイズ)
		template <class T>
		using vertex_array_data = pair<shared_ptr<vertex_data<T>>, dword>;
		...
		inline void draw_wire_frame(m3d::model_object<T>* model, const normal_color<T>& color) {
			...
			for (auto obj = model->object.begin(); obj != model->object.end(); ++obj) {
				...
				for (auto mat = obj->mat.begin(); mat != obj->mat.end(); ++mat) {
					...
					for (size_t i = 0; i < mat->vertex.size(); i += 6)
...
namespace iml {
	namespace i_f {
		...
		class light :public singleton<light<T, N>> {
			// Tはdoubleかfloatでなければならない
			static_assert(is_same_v<T, double> || is_same_v<T, float>, "T must be float or double.");
		...
		public:
			...
			void set_position(const vector<T, 4>& p) { position = p; glLightfv(GL_LIGHT0 + N, GL_POSITION, &position[0]); }
			...
			void set_direction(const vector3<T>& d) { direction = d; glLightfv(GL_LIGHT0 + N, GL_SPOT_DIRECTION, &direction[0]); }
		...
		public:
			...
			shadow_mapping(size_t w, size_t h, const CharT* vtx_file, const CharT* frag_file)
			...
			void create_shadow(F f) {
				// ビューポートエリアを合わせる
				auto client = interface::inst()->get_client_area();
				interface::inst()->set_client_area(rect<iml::int_t>(0, render->width(), render->height(), 0));

				// テクスチャ変換行列の構成
				//  個の入力はバグ
				tex_mat = matrix4<float>(
					0.5, 0, 0, 0.5
					, 0, 0.5, 0, 0.5
					, 0, 0, 0.5, 0.5
					, 0, 0, 0, 1)
					*get_projection_matrix()*get_view_matrix();
				...
				// ビュー行列を元に戻す
				interface::inst()->set_client_area(client);
			}
			// 影の描画
			void start() {
				// カメラから見たとき
			...
			size_t width() const { return render->width(); }
			size_t height() const { return render->height(); }
