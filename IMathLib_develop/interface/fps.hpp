...
namespace iml {
	namespace i_f {
		...
		public:
			...
			constexpr fps_control(const T& fps, imsize_t sample) : cnt(1), now_fps(0), fps(fps), N(sample) {}
			...
			//フレームカウント取得
			imsize_t frame() { return cnt; }

			//fpsと計測のサンプル数の変更
			void set_fps(const T& _fps, imsize_t _sample) {
				fps = _fps; N = _sample;
			}
		};
	}
