...
namespace iml {
	...
	public:
		...
		struct LZSS_ENCODE_INFO {
			imsize_t		EncodeCode;			//圧縮情報の開始を示す数値
		};
	...
	private:
		...
		imsize_t						_p_original;	//オリジナルデータの位置(データ取り出し時に使用)
		imsize_t						_p_press;		//圧縮データの位置(データ取り出し時に使用)
	...
	public:
		...
		//オリジナルデータを追加する
		void original_add(void* data, imsize_t size) {
			unsigned char* p = (unsigned char*)data;
			for (imsize_t i = 0; i < size; ++i, ++p)
				_original.push_back(*p);
		}
		//圧縮データを追加する
		void press_add(void* data, imsize_t size) {
			unsigned char* p = (unsigned char*)data;
			for (imsize_t i = 0; i < size; ++i, ++p) {
				_press.push_back(*p);
			}
		...
		//オリジナルデータの現在位置の変更
		bool original_pos(imsize_t p) { if ((_original.size() - 1) < p) return false; _p_original = p; return true; }
		//圧縮データの現在位置の変更
		bool press_pos(imsize_t p) { if ((_press.size() - 1) < p) return false; _p_press = p; return true; }

		//オリジナルデータの取り出し
		void original_get(void* data, imsize_t size);
		//圧縮データの取り出し
		void press_get(void* data, imsize_t size);

		//オリジナルデータのコンバート
		imsize_t original_convert();
		//圧縮データのコンバート
		imsize_t press_convert();

		//オリジナルデータのサイズ
		imsize_t original_size() { return _original.size(); }
		//圧縮データのサイズ
		imsize_t press_size() { return _press.size(); }
		...
		//エンコード
		static shared_array<unsigned char> encode(const unsigned char* binary, imsize_t byte) {
			__int32 encode_code;
		}

		//デコード
		static shared_array<unsigned char> decode(const unsigned char* binary, imsize_t byte) {
			
		}
	};

	//データのコンバート
	inline imsize_t lzss::original_convert() {

		imsize_t SrcSizeCounter = 0;
		int EncodeCode;
		...
		{
			imsize_t counter_table[256] = {};		//1バイトで表現できる数値は0~255
			imsize_t *MinNum = nullptr;

			//出現回数をカウント
			for (imsize_t i = 0; i < _original.size(); ++i) counter_table[_original[i]] ++;
			//最も出現頻度の低いコードの取得
	...
	inline imsize_t lzss::press_convert() {

		imsize_t PressSizeCounter = 0;
