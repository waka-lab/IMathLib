...
namespace iml {
	...
	class _Slide_window {
		unsigned					m_top = 0;
		const imsize_t				m_maxbufferSize;
	...
	public:
		...
		unsigned char& at(imsize_t index) {
			//本当はバッファサイズを２の乗数に固定することで
			//「%」を「&」の計算に置き換えられる
			return m_buffer.at((index + m_top) % m_maxbufferSize);
		}
		const unsigned char& at(imsize_t index) const {
			return m_buffer.at((index + m_top) % m_maxbufferSize);
		}
		imsize_t size() const { return m_buffer.size(); }
	};

	//長さ/距離に該当するデータパターンを返す
	//長さ > 距離に対応できるようにする
	//新しくpush_back()された値ほど、距離は短い
	inline shared_array<unsigned char> get_pattern(const _Slide_window& slide, imsize_t length, imsize_t distance) {
		shared_array<unsigned char> vec;
		vec.resize(length);
		//スライド窓の後ろから距離分の長さを循環させて代入
		for (imsize_t i = 0; i < length; ++i) vec[i] = slide.at(slide.size() - (distance - (i % distance)));
		return vec;
	}
	...
	class deflate {
		...
		//拡張ビット分の読み出し
		static imsize_t read_extra_value(_Bit_stream& bitstream, imsize_t base, imsize_t extra_bit) {
			if (extra_bit == 0) return base;
			return base + bitstream.get_range(extra_bit);
		}

		//スライド窓から拝借するパターンの長さ情報を読み出す
		static imsize_t read_length_code(imsize_t code, _Bit_stream& bitstream){
			constexpr imsize_t code_begin = 257;
			constexpr imsize_t code_end = 286;
			assert(code >= code_begin && code < code_end);

			//テーブルの宣言
			static pair<imsize_t, imsize_t> code_table[] = {
				//最小の長さ/拡張ビット数
				{ 3,0 },
				{ 4,0 },
				{ 5,0 },
			...
			pair<imsize_t, imsize_t>& info = code_table[code - code_begin];
			return read_extra_value(bitstream, info.first, info.second);
		}

		//スライド窓の参照開始地点(距離)の情報を読み出す
		static imsize_t read_distance_code(imsize_t code, _Bit_stream& bitstream){
			constexpr imsize_t code_end = 30;
		...
		static _Prefix_code_tree make_fixed_huffman_tree(){
			...
			//0 - 143 -> 8bit
			//[0011 0000] ～ [10111111]
			for (imsize_t i = 0; i <= 143; ++i) {
				int key = 0x30 + i;
				fixedTree.Entry<8>(key, i);
			}
			//144 - 255 -> 9bit
			//[110010000] ～ [111111111]
			for (imsize_t i = 0; i <= (255 - 144); ++i) {
				int key = 0x190 + i;
				fixedTree.Entry<9>(key, i + 144);
			}
			//256 - 279 -> 7bit
			//[0000000] ～ [0010111]
			for (imsize_t i = 0; i <= (279 - 256); ++i) {
				int key = 0x00 + i;
				fixedTree.Entry<7>(key, i + 256);
			}
			//280 - 287 -> 8bit
			//[11000000] ～ [11000111]
			for (imsize_t i = 0; i <= (287 - 280); ++i) {
				int key = 0xC0 + i;
		...
		static void _Decode_fixed_huffman(_Bit_stream& bitstream, _Slide_window& slide, shared_array<unsigned char>& resultbuffer){
			...
			while (iml::_Bit_tree_decode(bitstream, tree, &val)) {
				...
				//長さ情報
				imsize_t length = read_length_code(val, bitstream);
				//距離情報(5bit固定)
				imsize_t extra_value = bitstream.get_coded_range(5);
				imsize_t distance = read_distance_code(extra_value, bitstream);
		...
		static _Prefix_code_tree MakeNormalizedHuffmanTree(std::array<imsize_t, NUM_CODE>& codeLanArray){
			static_assert(NUM_CODE > 0, "不正な配列サイズ");

			// 事前に最大の長さを調べておく
			constexpr auto CAPACITY_LENGTH = limits<imsize_t>::digits;
			const imsize_t   maxCodeLength = *std::max_element(codeLanArray.begin(), codeLanArray.end());
			assert(maxCodeLength <= CAPACITY_LENGTH);

			// 符号長別に出現する符号個数をカウント
			std::array<imsize_t, CAPACITY_LENGTH> codeLenCount{};
			for (imsize_t i = 0; i < NUM_CODE; ++i){
				auto length = codeLanArray[i];
				if (length > 0) codeLenCount[length - 1] += 1;
			}

			// 符号長別の最小の符号(最初に割り当てられる符号)を準備
			std::array<imsize_t, CAPACITY_LENGTH> allocateCodes{ 0 };
			for (imsize_t i = 1; i < maxCodeLength; ++i){
				// note:
				// 符号長が一つ小さいグループのなかで
				// 未割当てになる符号を１bit++したものを
				// この符号長グループの符号割り当てに使用する
				// ---> 瞬時複合可能な条件を満たす
				imsize_t prevEndCode = allocateCodes[i - 1] + codeLenCount[i - 1];
		...
		static _Prefix_code_tree ReadCodeLenCodeTree(_Bit_stream& bitstream, int numCodeLenCode){
			...
			// 全体のデータ数を減らす最適化のため?
			static const imsize_t indexSequence[] ={
				16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15
			};

			// 読み出されなかった要素は「0」になる
			std::array<imsize_t, 19> codeLenCodeLens{};
			for (imsize_t i = 0; i < numCodeLenCode; ++i){
		...
		//@brief 繰り返し長さを読み出す
		//-------------------------------------------------------------
		static unsigned ReadRunLength(imsize_t code, _Bit_stream& bitstream){
			const imsize_t code_begin = 16;
			const imsize_t code_end = 19;
			assert(code >= code_begin && code < code_end);

			// テーブルの宣言
			static const std::pair<imsize_t, imsize_t> TABLE[] = {
				// 最小の繰返し回数 / 拡張ビット数
				std::make_pair(3, 2),
				std::make_pair(3, 3),
				std::make_pair(11, 7),
		...
		static _Prefix_code_tree ReadCustomHuffmanTree(_Bit_stream& bitstream, imsize_t numRead, _Prefix_code_tree codeLenCodeTree){
			assert(numRead <= CAPACITY_LENGTH);
			std::array<imsize_t, CAPACITY_LENGTH> codeLenArray{};

			for (imsize_t index = 0; index < numRead; ++index) {
				...
				for (imsize_t j = 0; j < runLength; ++j) codeLenArray[index + j] = repeatVal;
				index += (runLength - 1);
			}
			// この符号長配列からハフマンツリーを作る
		...
		static void _Decode_non_compressed(_Bit_stream& bitstream, _Slide_window& slide, shared_array<unsigned char>& resultbuffer){
			//次のbyte境界位置に移動
			bitstream.skip_byte();
			imsize_t length = bitstream.get_range(16);
			imsize_t nlength = bitstream.get_range(16);

			if (length + nlength != 0xffff) throw std::runtime_error("非圧縮データサイズが不正です。");
			for (imsize_t i = 0; i < length; ++i) {
	...
	public:
		//デコード
		static shared_array<unsigned char> decode(const unsigned char* binary, imsize_t numByte, imsize_t slideWindowSize) {
