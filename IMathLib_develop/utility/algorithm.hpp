...
namespace iml {
	...
	struct _Sort {
		...
		//整数の自然対数の整数近似
		static constexpr imsize_t __log_int(imsize_t x) {
			imsize_t index = 0;			//冪指数
			double c = x;
			...
			while (c > 2) { c /= 2; ++index; }
			if (c >= 1.5) ++index;
			return static_cast<imsize_t>(index*0.69314718056);
		}
		...
		//イントロソート用のクイックソート
		static constexpr void __quick_sort(iterator first, iterator last, imsize_t size) {
	...
	struct _Sort<BidirectionalIterator, Predicate, bidirectional_iterator_tag> {
		...
		static constexpr void __quick_sort(iterator first, iterator last, imsize_t size, imsize_t dis) {
			if (dis <= size) return;
	...
	struct _Sort<ForwardIterator, Predicate, forward_iterator_tag> {
		...
		static constexpr void __sort(iterator first, iterator last) {
			imsize_t dis = distance(first, last);
			if (dis <= 1) return;
			for (imsize_t interval = dis * 10 / 13;; interval = interval * 10 / 13) {
...
namespace iml {
	...
	//イテレータ範囲からvを探索
	template <class InputIterator, class T, class Predicate = type_comparison<void>>
	inline constexpr InputIterator find(InputIterator first, InputIterator last, const T& v) {
		...
		for (; first != last; ++first)
			if (Predicate::eq(*first, v)) return first;
		return last;
	}
...
namespace iml {
	...
	inline constexpr OutputIterator copy_order(OutputIterator first1, InputIterator first2, imsize_t n) {
		static_assert(is_iterator<OutputIterator, output_iterator_tag>::value
			, "The type of iterator is different.");
		static_assert(is_iterator<InputIterator, input_iterator_tag>::value
			, "The type of iterator is different.");

		for (imsize_t i = 0; i < n; ++i, ++first1, ++first2)
			*first1 = *first2;
		return first1;
	}
	template <class OutputIterator, class InputIterator>
	...
	inline constexpr BidirectionalIterator1 copy_reverse_order(BidirectionalIterator1 first1, BidirectionalIterator2 first2, imsize_t n) {
	...
	inline constexpr OutputIterator copy_if(OutputIterator first1, InputIterator first2, imsize_t n, F f) {
		static_assert(is_iterator<OutputIterator, output_iterator_tag>::value
			, "The type of iterator is different.");
		static_assert(is_iterator<InputIterator, input_iterator_tag>::value
			, "The type of iterator is different.");

		for (imsize_t i = 0; i < n; ++i, ++first2)
