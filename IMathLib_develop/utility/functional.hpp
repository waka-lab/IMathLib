...
namespace iml {
	...
	//関数型の任意の位置の引数型の取得
	template <imsize_t N, class F>
	struct function_args_type { using type = at_type<N, typename is_function<F>::arg_types>; };
	template <imsize_t N, class R, class... Types>
	struct function_args_type<N, function<R(Types...)>> { using type = typename at_type<N, Types...>::type; };

	//可変長引数の任意の位置の値の取得
	template <imsize_t M, imsize_t N, bool flag = (M == N)>
	...
	template <imsize_t M, imsize_t N>
