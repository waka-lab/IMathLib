...
namespace iml {
	...
	template <class F, class, class Decay = decay_t<F>, bool = is_member_function_pointer_v<Decay>, bool = is_member_object_pointer_v<Decay>>
	struct invoker_impl;
	...
	struct invoker_impl<F, T, Decay, true, false> :
		conditional_t<is_base_of<typename is_member_function_pointer_impl<Decay>::class_type, decay_t<T>>::value,
		invoker_pmf_object, invoker_pmf_pointer> {};
	//メンバデータへのポインタのとき(FのクラスがTの基底なら通常の呼び出しでそうでないならポインタ)
	template <class F, class T, class Decay>
	struct invoker_impl<F, T, Decay, false, true> :
		conditional_t<is_base_of<typename is_member_object_pointer_impl<Decay>::class_type, decay_t<T>>::value,
		invoker_pmd_object, invoker_pmd_pointer> {};
	//関数オブジェクト
	template <class F, class T, class Decay>
