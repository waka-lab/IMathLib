...
namespace iml {
	...
	using ChildIndexArray = std::array<imsize_t, 2>;
	...
	class _Bit_stream {
		...
		imsize_t				_next_bit = 0;
		imsize_t				_next_byte = 0;
	public:
		constexpr _Bit_stream(const unsigned char* binary, imsize_t byte) :binary(binary), size(byte) {}
		...
		imsize_t get_range(imsize_t n) {
			imsize_t bit = 0;
			for (imsize_t i = 0; i < n; ++i) bit |= (get() << i);
			return bit;
		}
		//ビット列をロード
		//符号化されたデータは最上位ビットから順にパックされている
		imsize_t get_coded_range(imsize_t numbit) {
			imsize_t bit = 0;
			for (imsize_t i = 0; i < numbit; ++i) {
				bit <<= 1;
				bit |= get();
			}
			return bit;
		...
		imsize_t now_bit() const { return _next_bit; }
		imsize_t now_byte() const { return _next_byte; }
	...
	class _Basic_prefix_tree {
		//無効なインデックス
		static constexpr imsize_t invalid_index = -1;
	public:
		...
		public:
			...
			imsize_t ChildIndex(int bit) const {
		...
		private:
			...
			imsize_t GetNextIndex(int bit) const {
	...
	private:
		...
		imsize_t				m_nodeTop = invalid_index;
		...
		std::pair<Node&, bool> CreateNodeIfNotFound(std::bitset<numBit> bits, unsigned numRead) {
			...
			for (; i >= 0; --i) {
				imsize_t newIndex = static_cast<imsize_t>(m_nodeList.size());
				m_nodeList.push_back(Node{});
...
namespace iml {
	...
	using _Prefix_code_tree = _Basic_prefix_tree<imsize_t>;
	...
	public:
		...
		static shared_array<unsigned char> encode(const unsigned char* binary, imsize_t byte) {
			...
			imsize_t sum_table[256] = {};				//ASCIIコードの和テーブル

			//和のテーブルの作成
			for (imsize_t i = 0; i < byte; ++i) ++sum_table[binary[i]];
