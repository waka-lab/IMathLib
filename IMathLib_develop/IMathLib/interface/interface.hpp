...
namespace iml {
	namespace i_f {
		...
		//汎用ステートの有効
		template <imsize_t FirstIndex>
		inline void enable_state() {
			glEnable(FirstIndex);
		}
		template <imsize_t FirstIndex, imsize_t SecondIndex, imsize_t... Index>
		inline void enable_state() {
			glEnable(FirstIndex);
		...
		class interface_control :public singleton<interface_control> {
			...
			rect<imint_t>		client_area;			//仮想クライアント領域(ビューポート領域)
...
/*#ifdef _IMATH_INTERFACE_IMAGE_
			...
			void init() {
...
#ifdef _IMATH_INTERFACE_AUDIO_
				{
					...
					imsize_t channel_sum = _IMATH_INTERFACE_CHANNEL_NUM_;
					if (channel_sum%_IMATH_INTERFACE_SE_NAM_ != 0)
						channel_sum += _IMATH_INTERFACE_SE_NAM_ - channel_sum % _IMATH_INTERFACE_SE_NAM_;
					//一つあたりのチャネルの個数
					imsize_t channel_1 = channel_sum / _IMATH_INTERFACE_SE_NAM_;
					//ミキシングチャネルを確保する
					Mix_AllocateChannels(channel_sum);
					//各チャンネルを登録
					for (imsize_t i = 0; i < _IMATH_INTERFACE_SE_NAM_; ++i)
			...
			void set_client_area(const rect<imint_t>& r) {
				client_area = r;
				glViewport(r.left, r.top, r.right - r.left, r.bottom - r.top);
			}
			//現在設定されているクライアント領域の取得
			const rect<imint_t>& get_client_area() { return client_area; }
		...
		public:
			window(imsize_t w, imsize_t h, Uint32 flag = SDL_WINDOW_HIDDEN) :window("IMathLib_interface window", w, h) {}
			window(const char* title, imsize_t w, imsize_t h, Uint32 flag = SDL_WINDOW_HIDDEN)
				:active_event([]() {})
