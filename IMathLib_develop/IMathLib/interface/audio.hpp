	...
	namespace i_f {
		...
		class audio {
			...
			struct holder_base {
				...
				virtual	void play(imsize_t loop, imsize_t ms) = 0;		//loop回ループしてms秒のフェードで再生
				virtual	void pause() = 0;							//一時停止
				virtual void resume() = 0;							//一時停止解除
				virtual	void reset(imsize_t ms) = 0;					//msのフェードでリセット
			...
			struct holder_se :holder_base {
				...
				virtual	void reset(imsize_t ms) { Mix_FadeOutChannel(channel, ms); }
		...
		public:
			static const imsize_t bgm = 0;			//BGM識別
			static const imsize_t se = 1;				//SE識別(1～)
			...
			//全ての音楽のリセット
			static void reset_all(imsize_t ms) {
