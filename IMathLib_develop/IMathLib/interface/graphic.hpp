...
namespace iml {
	namespace i_f {
		...
		public:
			//テクスチャ拡大縮小時の補間
			static const imsize_t nearest = GL_NEAREST;			//最近傍補間
			static const imsize_t linear = GL_LINEAR;			//双線形補間

			constexpr texture() {}
			texture(const SDL_Surface *img, imsize_t interpolation = nearest) {
				int				internal_format;
				int				pixel_format;
			...
			texture(const img::image *img, imsize_t interpolation = nearest) {
				int				internal_format;
				int				pixel_format;
			...
			imsize_t id() const { return tex_id; }
			imsize_t width() const { return w; }
			imsize_t height() const { return h; }
		};

		//シェーダークラス
		class shader {
			imsize_t shader_id;			//シェーダーの管理id
		public:
			static constexpr imsize_t vertex = 0;		//バーテクスシェーダー
			static constexpr imsize_t fragment = 1;		//フラグメントシェーダー
			static constexpr imsize_t geometry = 2;		//ジオメトリシェーダー

			shader(const char* str, imsize_t ident) {
			...
			const imsize_t id() const { return shader_id; }

		};

		//シェアポインタとして管理するためのプログラムオブジェクトクラス
		class program_object {
			imsize_t po_id;			//プログラムオブジェクトのid
		...
		public:
			...
			void attach(imsize_t first) { glAttachShader(po_id, first); }
			template <class... Args>
			void attach(imsize_t first, const Args&... shaders) { glAttachShader(po_id, first); attach(shaders...); }
			//デタッチ
			void dettach() {}
			...
			void dettach(imsize_t first) { glDetachShader(po_id, first); }
			template <class... Args>
			void dettach(imsize_t first, const Args&... shaders) { glDetachShader(po_id, first); dettach(shaders...); }
			//シェーダのリンク
			void link() {
				int result = GL_FALSE;
				glLinkProgram(po_id);
			...
			void set_variable(const char* str, imsize_t size, int* v) { glUniform1iv(get_variable(str), size, v); }
			void set_variable(const char* str, imsize_t size, float* v) { glUniform1fv(get_variable(str), size, v); }
		...
		protected:
			imsize_t	buffer_handle = 0;		//バッファハンドル
			imsize_t	obj_id = 0;				//フレームバッファオブジェクトid

			imsize_t	w, h;
		public:
			constexpr frame_buffer_object_base(imsize_t w, imsize_t h) :w(w), h(h) {}
			virtual ~frame_buffer_object_base() = 0 {}

			imsize_t handle() const { return buffer_handle; }
			imsize_t id() const { return obj_id; }
		...
		class color_frame_buffer_object :public frame_buffer_object_base {
			imsize_t render_buffer;			//zバッファ
		public:
			color_frame_buffer_object(imsize_t w, imsize_t h, bool alpha = true, bool depth = true) :frame_buffer_object_base(w, h) {
		...
		public:
			render_frame_buffer_object(imsize_t w, imsize_t h) :frame_buffer_object_base(w,h){
		...
		template <imsize_t N = 0>
		inline void set_coord_pointer(imsize_t tex) {
			static const vertex2<double> tex_coord(0, 0, 1, 1);
			glMultiTexCoordPointerEXT(GL_TEXTURE0 + N, 2, GL_DOUBLE, 0, tex_coord.vtx);
		}
		template <imsize_t N = 0, class... Args>
		inline void set_coord_pointer(imsize_t tex, const Args&... args) {
			static const vertex2<double> tex_coord(0, 0, 1, 1);
			glMultiTexCoordPointerEXT(GL_TEXTURE0 + N, 2, GL_DOUBLE, 0, tex_coord.vtx);
		...
		//各テクスチャをバインドして描画
		template <imsize_t N = 0>
		inline void draw_bind_texture(imsize_t n, imsize_t tex) {
			glActiveTexture(GL_TEXTURE0 + N);
			glBindTexture(GL_TEXTURE_2D, tex);
			glDrawArrays(GL_QUADS, 0, 4 * n);
		}
		template <imsize_t N = 0, class... Args>
		inline void draw_bind_texture(imsize_t n, imsize_t tex, const Args&... args) {
			glActiveTexture(GL_TEXTURE0 + N);
			glBindTexture(GL_TEXTURE_2D, tex);
			glDrawArrays(GL_QUADS, 0, 4 * n);
		...
		template <imsize_t N = 0>
		inline void draw_bind_texture(imsize_t n, const frame_buffer_object_base* tex) {
			glActiveTexture(GL_TEXTURE0 + N);
			glBindTexture(GL_TEXTURE_2D, tex->handle());
			glDrawArrays(GL_QUADS, 0, 4 * n);
		}
		template <imsize_t N = 0, class... Args>
		inline void draw_bind_texture(imsize_t n, const frame_buffer_object_base* tex, const Args&... args) {
			glActiveTexture(GL_TEXTURE0 + N);
			glBindTexture(GL_TEXTURE_2D, tex->handle());
			glDrawArrays(GL_QUADS, 0, 4 * n);
			draw_bind_texture(args...);
		...
		template <imsize_t N = 0>
		inline void draw_bind_texture(imsize_t n, const texture* tex) {
			glActiveTexture(GL_TEXTURE0 + N);
			glBindTexture(GL_TEXTURE_2D, tex->id());
			glDrawArrays(GL_QUADS, 0, 4 * n);
		}
		template <imsize_t N = 0, class... Args>
		inline void draw_bind_texture(imsize_t n, const texture* tex, const Args&... args) {
			glActiveTexture(GL_TEXTURE0 + N);
			glBindTexture(GL_TEXTURE_2D, tex->id());
			glDrawArrays(GL_QUADS, 0, 4 * n);
			draw_bind_texture(args...);
		...
		inline void bind_texture(imsize_t tex_id, imsize_t tex_unit = 0) {
			glActiveTexture(GL_TEXTURE0 + tex_unit);
			glBindTexture(GL_TEXTURE_2D, tex_id);
		}
		inline void bind_texture(const texture* tex, imsize_t tex_unit = 0) {
			glActiveTexture(GL_TEXTURE0 + tex_unit);
			glBindTexture(GL_TEXTURE_2D, tex->id());
		}
		inline void bind_texture(const frame_buffer_object_base* tex, imsize_t tex_unit = 0) {
			glActiveTexture(GL_TEXTURE0 + tex_unit);
			glBindTexture(GL_TEXTURE_2D, tex->handle());
		}
		//テクスチャ座標配列のセット
		template <imsize_t Byte = 0>
		inline void set_texture_coord_array(const float* coord, imsize_t tex_unit = 0) {
			glMultiTexCoordPointerEXT(GL_TEXTURE0 + tex_unit, 2, GL_FLOAT, Byte, coord);
		}
		template <imsize_t Byte = 0>
		inline void set_texture_coord_array(const double* coord, imsize_t tex_unit = 0) {
			glMultiTexCoordPointerEXT(GL_TEXTURE0 + tex_unit, 2, GL_DOUBLE, Byte, coord);
		}
		//頂点座標配列のセット(dimは次元)
		template <imsize_t Byte = 0>
		inline void set_vertex_array(imsize_t dim, const float* vtx) {
			glVertexPointer(dim, GL_FLOAT, Byte, vtx);
		}
		template <imsize_t Byte = 0>
		inline void set_vertex_array(imsize_t dim, const double* vtx) {
		...
		template <imsize_t FirstVtxIndex, imsize_t SecondVtxIndex, imsize_t... VtxIndex>
		inline void enable_array_state() {
			glEnableClientState(FirstVtxIndex);
		...
		inline void draw_texture(imsize_t first, imsize_t cnt, imsize_t tex_unit = 0) {
		...
		template <imsize_t Mode, class VtxIndexTuple, class Texture>
		inline void draw_texture(Texture tex, imsize_t first, imsize_t cnt, imsize_t tex_unit = 0) {
			bind_texture(tex, tex_unit);
			draw_texture<Mode, VtxIndexTuple>(first, cnt, tex_unit);
		}
		template <imsize_t Mode, class VtxIndexTuple, imsize_t VtxByte, imsize_t CoordByte, class Texture, class Float>
		inline void draw_texture(imsize_t dim, Texture tex, const Float* vtx, const Float* coord, imsize_t first, imsize_t cnt, imsize_t tex_unit = 0) {
		...
		//インデックス指定verのテクスチャ描画
		template <imsize_t Mode, class VtxIndexTuple>
		inline void draw_texture(const unsigned int* index, imsize_t cnt, imsize_t tex_unit = 0) {
			_Enable_array_state(VtxIndexTuple());
		...
		template <imsize_t Mode, class VtxIndexTuple>
		inline void draw_texture(const unsigned short* index, imsize_t cnt, imsize_t tex_unit = 0) {
			_Enable_array_state(VtxIndexTuple());
		...
		template <imsize_t Mode, class VtxIndexTuple>
		inline void draw_texture(const unsigned char* index, imsize_t cnt, imsize_t tex_unit = 0) {
			_Enable_array_state(VtxIndexTuple());
		...
		template <imsize_t Mode, class VtxIndexTuple, class Texture, class IndexType>
		inline void draw_texture(Texture tex, const IndexType* index, imsize_t cnt, imsize_t tex_unit = 0) {
			bind_texture(tex, tex_unit);
		...
		//図形描画
		template <imsize_t Mode, class VtxIndexTuple>
		inline void draw_figure(imsize_t first, imsize_t cnt) {
			_Enable_array_state(VtxIndexTuple());
		...
		template <imsize_t Mode, class VtxIndexTuple, imsize_t VtxByte, class Float>
		inline void draw_figure(imsize_t dim, const Float* vtx, imsize_t first, imsize_t cnt) {
			set_vertex_array<VtxByte>(dim, vtx);
			draw_figure<Mode, VtxIndexTuple>(first, cnt);
		}
		//インデックス指定verの図形描画
		template <imsize_t Mode, class VtxIndexTuple>
		inline void draw_figure(const unsigned int* index, imsize_t cnt) {
			_Enable_array_state(VtxIndexTuple());
		...
		template <imsize_t Mode, class VtxIndexTuple>
		inline void draw_figure(const unsigned short* index, imsize_t cnt) {
			_Enable_array_state(VtxIndexTuple());
		...
		template <imsize_t Mode, class VtxIndexTuple>
		inline void draw_figure(const unsigned char* index, imsize_t cnt) {
			_Enable_array_state(VtxIndexTuple());
