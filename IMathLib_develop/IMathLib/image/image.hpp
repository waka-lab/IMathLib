...
namespace iml {
	...
	namespace img {
		...
		private:
			//4の倍数に切り上げるの関数
			static imsize_t ceil4(imsize_t n) {
				return (n & 3) ? (((n >> 2) + 1) << 2) : (n);
			}
		public:
			const imsize_t w, h;				//幅、高さ
			imsize_t real_w;					//実際の幅のビット長(4の倍数)
			imsize_t color;						//色数(3～で最初の3色はRGBで基本的に3か4のみ)
			unsigned char* pixels;				//データ配列(左上から右下へ[R,G,B,...])

			image(imsize_t width, imsize_t height, imsize_t n)
			...
			//任意のピクセルに対するアクセサ
			const unsigned char* operator()(imsize_t width, imsize_t height) const { return pixels + height*(real_w) + width*color; }
			unsigned char* operator()(imsize_t width, imsize_t height) { return pixels + height*(real_w) + width*color; }

			//矩形領域の構造体
			struct rect {
				imsize_t  left, right, bottom, top;
			};

			//補間法を示すの定数
			static const imsize_t nearest = 0;				//最近傍補間
			static const imsize_t linear = 1;				//双線形補間
		};
		...
		inline image* load_png(const unsigned char* p) {
			...
			imsize_t comp_size = 0;			//圧縮データのサイズ
			...
			static const auto paeth_predictor = [](imint_t a, imint_t b, imint_t c) {
				imint_t p = a + b - c;
				imint_t pa = abs(p - a);
				imint_t pb = abs(p - b);
				imint_t pc = abs(p - c);
			...
			//差分の復元
			static const auto restore_difference = [](imint_t a, imint_t b) {
				return (a + b) & 0xFF;
			};
			//上の色の取得
			static const auto get_up = [](image* img, imsize_t width, imsize_t height, imsize_t k) {
				return (height == 0) ? 0 : (*img)(width, height - 1)[k];
			};
			//左の色の取得
			static const auto get_left = [](image* img, imsize_t width, imsize_t height, imsize_t k) {
				return (width == 0) ? 0 : (*img)(width - 1, height)[k];
			};
			//左上の色の取得
			static const auto get_up_left = [](image* img, imsize_t width, imsize_t height, imsize_t k) {
