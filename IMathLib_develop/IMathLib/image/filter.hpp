...
namespace iml {
	namespace img {
		...
		inline image* trimming(const image* img, const image::rect& rect) {
			...
			//全ての成分のコピー
			for(imsize_t i=rect.left;i<rect.right;++i)
				for (imsize_t j = rect.top; j < rect.bottom; ++j)
					for (imsize_t k = 0; k < img->color; ++k)
						(*result)(i - rect.left, j - rect.top)[k] = (*img)(i, j)[k];
			return result;
		}
		//サイズの変更(双線形補間か最近傍補間の選択)
		inline image* resize(const image* img, imsize_t width, imsize_t height, imsize_t interpolation = image::nearest) {
			image* result;
			...
			//逆変換により実装
			for (imsize_t i = 0; i < width; ++i) {
				double vicinity_w = rate_w*i;
				for (imsize_t j = 0; j < height; ++j) {
					//近傍点の算出
					double vicinity_h = rate_h*j;
					if (interpolation == image::nearest) {
						//最近傍点は四捨五入で算出
						imsize_t round_w = static_cast<imsize_t>(round(vicinity_w)), round_h = static_cast<imsize_t>(round(vicinity_h));
						for (imsize_t k = 0; k < img->color; ++k)
							(*result)(i, j)[k] = (*img)(round_w, round_h)[k];
					}
					else if (interpolation == image::linear) {
						//整数への丸めの取得
						imsize_t w = static_cast<imsize_t>(vicinity_w), h = static_cast<imsize_t>(vicinity_h);
						//小数部のみの取得
						double dw = vicinity_w - w, dh = vicinity_h - h;
						//4方で重みを付けて足す
						for (imsize_t k = 0; k < img->color; ++k)
							(*result)(i, j)[k] = static_cast<imint_t>((1 - dw)*(1 - dh)*(*img)(w, h)[k] + dw*(1 - dh)*(*img)(w + 1, h)[k]
		...
		inline image* flip_vertical(const image* img) {
			...
			//全ての成分のコピー
			for (imsize_t i = 0; i < img->w; ++i)
				for (imsize_t j = 0; j < img->h; ++j)
					for (imsize_t k = 0; k < img->color; ++k)
		...
		//回転(ラジアン,回転の中心,補間法)
		inline image* rotation(const image* img, double radian, imint_t center_w, imint_t center_h, imsize_t interpolation = image::nearest) {
			image* result;
			result = new image(img->w, img->h, img->color);
			if (!result->reserve()) { delete result; return nullptr; }
	...
	namespace img {
		namespace filter {
			...
			inline void grayscale(image *img) {
				...
				for (imsize_t i = 0, k = img->h*img->w; i < k; ++i) {
					color = (p[0] + p[1] + p[2]) / 3;
					p[0] = p[1] = p[2] = color;
					p += img->color;
				}
			...
			inline void gamma(image *img, double g) {
				double inv = (g <= 0) ? (limits<double>::max)() : 1 / g;
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t j = 0; j < img->h; ++j)
						for (imsize_t k = 0; k < 3; ++k)
			...
			//モザイク処理(全ての色の成分について)
			inline void mosaic(image *img, imsize_t size) {
				imsize_t sum;

				for (imsize_t i = 0; i < img->w; i += size) {
					for (imsize_t j = 0; j < img->h; j += size) {
						for (imsize_t k = 0; k < img->color; ++k) {
							sum = 0;
							//和を算出
							for (imsize_t l = 0, _l = (iml::min)(img->w - i, size); l < _l; ++l)
								for (imsize_t m = 0, _m = (iml::min)(img->h - j, size); m < _m; ++m)
									sum += (*img)(i + l, j + m)[k];
							//平均
			...
			inline void rgb(image *img, imint_t r, imint_t g, imint_t b) {
				imint_t buf;
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t j = 0; j < img->h; ++j) {
						//値を加えて数値補正して代入
			...
			//RGBをHSVに変換
			inline void rgb_to_hsv(imint_t *hsv, const unsigned char *rgb) {
				unsigned char rgb_max = *max_element(rgb, &rgb[3]), rgb_min = *min_element(rgb, &rgb[3]);
				hsv[2] = rgb_max;
				hsv[1] = static_cast<imint_t>(255. * (rgb_max - rgb_min) / hsv[2]);
				if (rgb_max == rgb[0]) hsv[0] = static_cast<imint_t>(60. * (rgb[2] - rgb[1]) / (rgb_max - rgb_min));
				else if (rgb_max == rgb[1]) hsv[0] = static_cast<imint_t>(60. * (2 + static_cast<double>(rgb[0] - rgb[2]) / (rgb_max - rgb_min)));
				else if (rgb_max == rgb[2]) hsv[0] = static_cast<imint_t>(60. * (4 + static_cast<double>(rgb[1] - rgb[0]) / (rgb_max - rgb_min)));
				if (hsv[0] < 0) hsv[0] += 360;
			}
			//HSVをRGBに変換
			inline void hsv_to_rgb(unsigned char *rgb, const imint_t *hsv) {
				imint_t hi = hsv[0] / 60;
			...
			inline void hsv(image *img, imint_t h, imint_t s, imint_t v) {
				imint_t buf[3];
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t j = 0; j < img->h; ++j) {
						//元データのrgbをhsvに変換
			...
			inline void smooth_horizon_average(image *img, imsize_t n, imint_t nk = 1) {
				...
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t j = 0; j < img->h; ++j)
						for (imsize_t k = 0; k < img->color; ++k) {
							sum = 0;
							//和の算出
							for (imint_t l = -static_cast<imint_t>((iml::min)(i, half_n)), _l = (iml::min)(img->w - i, half_n + 1); l < _l; ++l)
								sum += (*img)(i + l, j)[k];
							//重みの代入
							sum += (*img)(i, j)[k] * (nk - 1);
							//平均と代入
			...
			inline void smooth_vertical_average(image *img, imsize_t n, imint_t nk = 1) {
				image* buf = new image(img->w, img->h, img->color);
				imsize_t half_n = (n >> 1);
				imsize_t sum;
				if ((n & 1) == 0) ++n;			//奇数にする
				...
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t j = 0; j < img->h; ++j)
						for (imsize_t k = 0; k < img->color; ++k) {
							sum = 0;
							//和の算出
							for (imint_t l = -static_cast<imint_t>((iml::min)(j, half_n)), _l = (iml::min)(img->h - j, half_n + 1); l < _l; ++l)
								sum += (*img)(i, j + l)[k];
							//重みの代入
							sum += (*img)(i, j)[k] * (nk - 1);
							//平均と代入
			...
			inline void smooth_average(image *img, imsize_t n, imint_t nk = 1) {
				image* buf = new image(img->w, img->h, img->color);
				imsize_t half_n = (n >> 1);
				imsize_t sum;
				if ((n & 1) == 0) ++n;			//奇数にする
				...
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t j = 0; j < img->h; ++j)
						for (imsize_t k = 0; k < img->color; ++k) {
							...
							//和の算出
							for (imint_t l = -static_cast<imint_t>((iml::min)(i, half_n)), _l = (iml::min)(img->w - i, half_n + 1); l < _l; ++l)
								for (imint_t m = -static_cast<imint_t>((iml::min)(j, half_n)), _m = (iml::min)(img->h - j, half_n + 1); m < _m; ++m)
									sum += (*img)(i + l, j + m)[k];
							//重みの代入
							sum += (*img)(i, j)[k] * (nk - 1);
							//平均と代入
			...
			//中央値フィルタ(nは奇数)
			inline void median(image *img, imsize_t n) {
				image* buf = new image(img->w, img->h, img->color);
				imsize_t half_n = (n >> 1);
				imsize_t array_n;				//データ配列用の配列番号
				imsize_t* data;					//データ配列
				if ((n & 1) == 0) ++n;			//奇数にする
				data = new imsize_t[n*n];
			...
			//膨張フィルタ(nは奇数)
			inline void dilation(image *img, imsize_t n) {
				image* buf = new image(img->w, img->h, img->color);
				imint_t hsv[3];
				imsize_t half_n = (n >> 1);
				if ((n & 1) == 0) ++n;			//奇数にする
				...
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t j = 0; j < img->h; ++j) {
						...
						imint_t v_max = 0;
						unsigned char *rgb_p
							= (*img)(i - static_cast<imint_t>((iml::min)(i, half_n)), j - static_cast<imint_t>((iml::min)(j, half_n)));
						for (imint_t l = -static_cast<imint_t>((iml::min)(i, half_n)), _l = (iml::min)(img->w - i, half_n + 1); l < _l; ++l)
							for (imint_t m = -static_cast<imint_t>((iml::min)(j, half_n)), _m = (iml::min)(img->h - j, half_n + 1); m < _m; ++m) {
			...
			//収縮フィルタ(nは奇数)
			inline void erosion(image *img, imsize_t n) {
				image* buf = new image(img->w, img->h, img->color);
				imint_t hsv[3];
				imsize_t half_n = (n >> 1);
				if ((n & 1) == 0) ++n;			//奇数にする
			...
			inline void horizon_first_derivative(image *img) {
				...
				//端以外の処理
				for (imsize_t i = 1; i < img->w - 1; ++i)
					for (imsize_t j = 0; j < img->h; ++j)
						for (imsize_t k = 0; k < 3; ++k)
							(*buf)(i, j)[k] = abs((*img)(i + 1, j)[k] - (*img)(i - 1, j)[k]);
				//端の処理
				for (imsize_t i = 0; i < img->h; ++i)
					for (imsize_t k = 0; k < 3; ++k) {
			...
			//横方向prewittフィルタ
			inline void horizon_prewitt(image *img, imsize_t n) {
				image* buf = new image(img->w, img->h, img->color);
				imint_t sum;
				imsize_t half_n = (n >> 1);
				if ((n & 1) == 0) ++n;			//奇数にする
				...
				for (imsize_t i = 0; i < img->h; ++i)
					for (imsize_t k = 0; k < 3; ++k) {
						sum = 0;
						//和の算出
						for (imint_t m = -static_cast<imint_t>((iml::min)(i, half_n)), _m = (iml::min)(img->h - i, half_n + 1); m < _m; ++m)
			...
			//縦方向prewittフィルタ
			inline void vertical_prewitt(image *img, imsize_t n) {
				image* buf = new image(img->w, img->h, img->color);
				imint_t sum;
				imsize_t half_n = (n >> 1);
				if ((n & 1) == 0) ++n;			//奇数にする
				...
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t k = 0; k < 3; ++k) {
						sum = 0;
						//和の算出
						for (imint_t m = -static_cast<imint_t>((iml::min)(i, half_n)), _m = (iml::min)(img->w - i, half_n + 1); m < _m; ++m)
			...
			inline void prewitt(image *img, imsize_t n) {
				image* buf = new image(img->w, img->h, img->color);
				imint_t sum[2];
				imsize_t half_n = (n >> 1);
				if ((n & 1) == 0) ++n;			//奇数にする
			...
			//横方向sobelフィルタ(s:加重係数)
			inline void horizon_sobel(image *img, imsize_t n, imsize_t s = 2) {
				image* buf = new image(img->w, img->h, img->color);
				imint_t sum;
				imsize_t half_n = (n >> 1);
				if ((n & 1) == 0) ++n;			//奇数にする
				...
				for (imsize_t i = 1; i < img->w - 1; ++i)
					for (imsize_t j = 0; j < img->h; ++j)
						for (imsize_t k = 0; k < 3; ++k) {
							sum = 0;
							//和の算出
							for (imint_t m = -static_cast<imint_t>((iml::min)(j, half_n)); m < 0; ++m)
								sum += (*img)(i + 1, j + m)[k] - (*img)(i - 1, j + m)[k];
							sum += ((*img)(i + 1, j)[k] - (*img)(i - 1, j)[k])*s;					//重みを付ける
							for (imint_t m = 1, _m = (iml::min)(img->h - j, half_n + 1); m < _m; ++m)
								sum += (*img)(i + 1, j + m)[k] - (*img)(i - 1, j + m)[k];
							//代入
				...
				for (imsize_t i = 0; i < img->h; ++i)
					for (imsize_t k = 0; k < 3; ++k) {
						sum = 0;
						//和の算出
						for (imint_t m = -static_cast<imint_t>((iml::min)(i, half_n)); m < 0; ++m)
							sum += (*img)(1, i + m)[k] - (*img)(0, i + m)[k];
						sum += ((*img)(1, i)[k] - (*img)(0, i)[k])*s;
						for (imint_t m = 1, _m = (iml::min)(img->h - i, half_n + 1); m < _m; ++m)
							sum += (*img)(1, i + m)[k] - (*img)(0, i + m)[k];
						//代入
			...
			//縦方向sobelフィルタ
			inline void vertical_sobel(image *img, imsize_t n, imsize_t s = 2) {
				image* buf = new image(img->w, img->h, img->color);
				imint_t sum;
				imsize_t half_n = (n >> 1);
				if ((n & 1) == 0) ++n;			//奇数にする
				...
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t j = 1; j < img->h - 1; ++j)
						for (imsize_t k = 0; k < 3; ++k) {
							sum = 0;
							//和の算出
							for (imint_t l = -static_cast<imint_t>((iml::min)(i, half_n)); l < 0; ++l)
								sum += (*img)(i + l, j + 1)[k] - (*img)(i + l, j - 1)[k];
							sum += ((*img)(i, j + 1)[k] - (*img)(i, j - 1)[k])*s;
							for (imint_t l = 1, _l = (iml::min)(img->w - i, half_n + 1); l < _l; ++l)
								sum += (*img)(i + l, j + 1)[k] - (*img)(i + l, j - 1)[k];
							//代入
							(*buf)(i, j)[k] = static_cast<unsigned char>(abs(sum));
						}
				...
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t k = 0; k < 3; ++k) {
						...
						for (imint_t m = 1, _m = (iml::min)(img->w - i, half_n + 1); m < _m; ++m)
							sum += (*img)(i + m, 1)[k] - (*img)(i + m, 0)[k];
						//代入
						(*buf)(i, 0)[k] = abs(sum);
			...
			inline void sobel(image *img, imsize_t n, imsize_t s = 2) {
				image* buf = new image(img->w, img->h, img->color);
				imint_t sum[2];
				imsize_t half_n = (n >> 1);
				if ((n & 1) == 0) ++n;			//奇数にする
			...
			//4方向ラプラシアンフィルタ
			inline void laplacian4(image *img, imsize_t n) {
				image* buf = new image(img->w, img->h, img->color);
				imint_t sum;
				if (!buf->reserve()) { delete buf; return; }
				...
				for (imsize_t i = 0; i < img->w; ++i)
					for (imsize_t j = 0; j < img->h; ++j)
						for (imsize_t k = 0; k < 3; ++k) {
							sum = 0;
							//4方向の情報(上、下、左、右)
							imint_t d[4] = { -static_cast<imint_t>((iml::min)(j, imsize_t(1))), (iml::min)(img->h - j, imsize_t(1))
								,-static_cast<imint_t>((iml::min)(i, imsize_t(1))),(iml::min)(img->w - i, imsize_t(1)) };
							//上下
