...
namespace iml {
	namespace rnd {
		...
		public:
			...
			//乱数を進める
			void discard(imsize_t k) { for (imsize_t i = 0; i < k; ++i) this->get(); }

			//計算回数
			imsize_t times() { return cnt; }
		...
		//Xorshift
		template <class T, imsize_t Shift1, imsize_t Shift2, imsize_t Shift3>
		class Xorshift32 :public random_base<T> {
		public:
			using type = T;
			using inner_type = unsigned __int32;
		private:
			static_assert((Shift1 <= (1 << limits<type>::digits) - 1) || (Shift2 <= (1 << limits<type>::digits) - 1)
				|| (Shift3 <= (1 << limits<type>::digits) - 1) || (limits<type>::digits <= limits<inner_type>::digits)
		...
		public:
			...
			constexpr type(max)() const { return (limits<type>::max)(); }
		...
		//メルセンヌ・ツイスタ
		template <class T, imsize_t w, imsize_t n, imsize_t m, imsize_t r, T a,
			imsize_t u, imsize_t s, imsize_t t, imsize_t l, T b, T c, T d,
			T f>
		...
		private:
			...
			template <imsize_t N, imsize_t X, type Result>
			struct _2_power_of_X :_2_power_of_X<N + 1, X, Result * 2> {};
			template <imsize_t X, type Result>
		...
		public:
			...
			void seed(type s) {
				x[0] = s;
				for (imsize_t i = 1; i < n; ++i) x[i] = (f * (x[i - 1] ^ (x[i - 1] >> 30)) + i);
				cnt = 0;
				make_bit_array();
			}
		...
		//mt19937の32ビット用
		using mersenne_twister_19937_32 = mersenne_twister<imuint32_t, 32, 624, 397, 31, 0x9908B0DF
			, 11, 7, 15, 18, 0x9D2C5680, 0xEFC60000, 0xFFFFFFFF
			, 1812433253>;
		//mt19937の64ビット用
		using mersenne_twister_19937_64 = mersenne_twister<imuint64_t, 64, 312, 156, 31, 0xB5026f5AA96619E9
