...
namespace iml {
	...
	class tree_map_iterator
		...
		using this_type = tree_map_iterator<T>;
	public:
	...
	class tree_map_container :public container_base<T, tree_map_iterator<T>, Allocator> {
	public:
		using typename container_base<T, tree_map_iterator<T>, Allocator>::value_type;
		using typename container_base<T, tree_map_iterator<T>, Allocator>::iterator;
		using typename container_base<T, tree_map_iterator<T>, Allocator>::const_iterator;
		using typename container_base<T, tree_map_iterator<T>, Allocator>::allocator_type;
	private:
		using this_type = tree_map_container;
	public:
		using key_type = typename remove_cv<typename T::first_type>::type;			//キーの型
		using mapped_type = typename T::second_type;								//要素の型
	protected:
		...
		static void _Clear(_Node* n, Allocator& _allo) {
			if (n->left != nullptr) {
				_Clear(n->left, _allo);
				_allo.destroy(iterator(n->left), 1);
			}
			if (n->right != nullptr) {
				_Clear(n->right, _allo);
				_allo.destroy(iterator(n->right), 1);
			}
		}
		//ノードの探索
		...
		static _Node* _Insert(const key_type& key, const mapped_type& v, _Node* n, Allocator& _allo) {
			...
			if (n->value.first < key) {
				if (n->right == nullptr) {
					n->right = _allo.create(1, key, v, n);
					return n->right;
				}
	...
	public:
		...
		_Node* insert(const key_type& key, const mapped_type& v = mapped_type()) {
			...
			//ルートは黒にして追加
			if (root == nullptr) {
				temp = _allo.create(1, key, v, nullptr);
				//rootならば無条件に黒
				temp->flag = false;
				++_size;
				return root = temp;
		...
		imsize_t size() const { return _size; }
	...
	//二分木による連想配列
	template <class Key, class T, class Compare = type_comparison<Key>, class Allocator = allocator<typename tree_map_iterator<pair<const Key, T>>::node, tree_map_iterator<pair<const Key, T>>>>
	...
	//二分木による連想配列
	template <class Key, class T, class Compare = type_comparison<Key>, class Allocator = allocator<typename tree_map_iterator<pair<const Key, T>>::node, tree_map_iterator<pair<const Key, T>>>>
	class shared_tree_map :public tree_map_container<pair<const Key, T>, Compare, Allocator> {
		shared_count<T, deallocator<T>::function>	sc;			//シェアカウンタ(clearで解放のためハンドルを渡す必要はない)
	public:
		constexpr shared_tree_map() :sc(nullptr, [&](T*) {clear(); }) {}
		~shared_tree_map() {}
		...
		shared_tree_map& operator=(const shared_tree_map& s) {
			if (s._p == this->_p) return *this;
			this->sc = s.sc;
			this->_allo = s._allo;
			this->_size = s._size;
		...
		shared_tree_map& operator=(shared_tree_map&& s) {
			if (s._p == this->_p) return *this;
			this->sc = s.sc;
			this->_allo = s._allo;
			this->_size = s._size;
