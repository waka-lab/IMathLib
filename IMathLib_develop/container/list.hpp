...
namespace iml {
	...
	public:
		...
		//前方探索
		imsize_t find(const T& key, imsize_t pos = 0) {
			if (_size <= pos) return npos;
			iterator temp = next(_header, pos + 1);
			for (imsize_t i = pos; i < _size; ++i, ++temp)
				if (*temp == key) return i;
			return npos;
		}
		//後方探索
		imsize_t rfind(const T& key, imsize_t pos = npos) {
			iterator temp = next(_header, (iml::min)(pos, this->_size) + 1);
			for (imsize_t i = (iml::min)(pos, this->_size); i != 0; --i, --temp)
				if (*temp == key) return i;
			return npos;
		}
	...
	public:
		...
		T& operator[](imsize_t index) noexcept { return (index < _size / 2) ? _header[index + 1] : *prev(_header, _size - index); }
		const T& operator[](imsize_t index) const noexcept { return (index < _size / 2) ? _header[index + 1] : *prev(_header, _size - index); }
	};
