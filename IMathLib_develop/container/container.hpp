...
namespace iml {
	...
	public:
		...
		//メモリ確保
		virtual _Pointer allocate(size_t n) = 0;
		//メモリ解法
		virtual void deallocate(Iterator p, size_t n) = 0;
		...
		template <class... Args>
		_Pointer create(size_t n, Args... args) {
			_Pointer temp = allocate(n);
			for (size_t i = 0; i < n; ++i) _Constructor(temp + i, args...);
			return temp;
		}
		//オブジェクトの破棄(デストラクタを呼び出しメモリを破棄する)
		void destroy(Iterator p, size_t n) {
			Iterator temp = p;
			if (n != 0) {
				for (size_t i = 0; i < n - 1; ++i) destructor(temp++);
				destructor(temp);
			}
			deallocate(p, n);
		}
	...
	public:
		...
		static constexpr size_t array_variable = 1;
		static constexpr size_t function = 2;
	private:
	...
	public:
		...
		static const size_t npos = -1;			//無効な位置
