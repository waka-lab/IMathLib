...
namespace iml {
	...
	inline imsize_t string_length(const CharT* str) {
		imsize_t temp = 0;
	...
	template <class CharT>
	inline const CharT* string_find(const CharT* str1, imsize_t len1, const CharT* str2, imsize_t len2) {
	...
	template <class CharT>
	inline const CharT* string_find(const CharT* str1, imsize_t len1, const CharT* str2) {
		return string_find(str1, len1, str2, string_length(str2));
	}
	template <class CharT>
	inline const CharT* string_find(const CharT* str1, const CharT* str2, imsize_t len2) {
		return string_find(str1, string_length(str1), str2, len2);
	}
	//後方から文字列の探索
	template <class CharT>
	inline const CharT* string_rfind(const CharT* str1, imsize_t len1, const CharT* str2, imsize_t len2) {
		if (len1 < len2) return nullptr;
	...
	template <class CharT>
	inline const CharT* string_rfind(const CharT* str1, imsize_t len1, const CharT* str2) {
		return string_rfind(str1, len1, str2, string_length(str2));
	}
	template <class CharT>
	inline const CharT* string_rfind(const CharT* str1, const CharT* str2, imsize_t len2) {
	...
	public:
		...
		string(const CharT* s) {
			imsize_t size = string_length(s);
			this->reserve(get_power_of_2(size + 1), s, size + 1);
		}
		string(const CharT* s, imsize_t size) {
			this->reserve(get_power_of_2(size + 1), s, size + 1);
		...
		//文字列の長さ取得
		imsize_t length() { return _use_size - 1; }
		...
		//前方探索(nposのときなし)
		imsize_t find(const string& key, imsize_t pos = 0) {
		...
		//文字列の一部の取得
		string substr(imsize_t pos, imsize_t size = npos) {
			imsize_t p = (iml::min)(this->_use_size, pos);
