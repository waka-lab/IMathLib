...
#include "IMathLib/math/liner_algebra/liner_mapping.hpp"
...
namespace iml {
	...
	struct math_function<matrix<T, M, N>> {
		...
		static type __identity_matrix() {
			return multiplicative<type>::identity_element();
		}
		//トレースの取得
		...
		//クロネッカー積
		template <imsize_t P, imsize_t Q>
		static matrix<T, M*P, N*Q> __kronecker_product(const type& ma1, const matrix<T, P, Q>& ma2) {
			...
			for (imsize_t i = 0; i < M; ++i)
				for (imsize_t j = 0; j < N; ++j)
					for (imsize_t k = 0; k < P; ++k)
						for (imsize_t l = 0; l < Q; ++l)
	...
	private:
		...
		static T __householder(T* v, imsize_t dim) {
			T	x_0 = 0;		//第一成分
			T	a;				//定数a

			//ベクトルの内積の取得
			for (imsize_t i = 0; i < dim; ++i)
				x_0 += v[i] * v[i];
		...
		//法線ベクトルに対する行列の更新
		static void __mult(matrix<T, M, N>* x, T *v, imsize_t dim) {
			imsize_t	offset = M - dim;			//行列とベクトルの次元のオフセット
			T		temp = 0;
			T*		g = new T[dim];

			//dの取得
			for (imsize_t i = offset; i < M; ++i) {
				g[i - offset] = 0;
				for (imsize_t j = offset; j < M; ++j)
		...
		static matrix<T, M, N> __tridiagonalization(matrix<T, M, N>* ma) {
			...
			for (imsize_t i = 0; i < M - 2; ++i) {
				v = vector<T, M>();
				//ハウスホルダー変換をする行の部分を取得
				--v_dim;
				for (imsize_t j = i + 1; j < M; ++j)
					v[j] = (*ma)[i][j];
		...
		//上三角行列と直交行列に変換
		static void __change_AQ(matrix<T, M, N>* a, matrix<T, M, N>* q, imsize_t dim) {
			T alpha, s, c;
			T temp;

			for (imsize_t i = 0; i < dim - 1; i++) {
				alpha = sqrt((*a)[i][i] * (*a)[i][i] + (*a)[i + 1][i] * (*a)[i + 1][i]);
				s = alpha ? (*a)[i + 1][i] / alpha : alpha;
				c = alpha ? (*a)[i][i] / alpha : alpha;

				for (imsize_t j = i + 1; j < dim; j++) {
...
namespace iml {
	...
	//ベクトルの直積(クロネッカー積)
	template <class T, imsize_t N, imsize_t P>
	...
	//クロネッカー積
	template <class T, imsize_t M, imsize_t N, imsize_t P, imsize_t Q>
