...
namespace iml {
	...
	struct _Ad_variable<T, N, index_imu_tuple<Indices...>> {
	...
	bottomup_ad<T, N> ad_variable(const T& x) {
		return _Ad_variable<T, N, typename index_imu_range<2, N>::type>::__ad_variable(x);
	}
...
namespace iml {
	...
	class automatic_partial_differentiation<R(Args...), D>{
		...
		class make_differentiation_struct<index_imu_tuple<Indices...>, Orders...> :public make_differentiation_type<sizeof...(Indices), Indices>... {
			...
			struct function_inclusion_struct {
				static void __function_inclusion(make_differentiation_struct<index_imu_tuple<Indices...>, Orders...>& mds) {
		...
		class make_differentiation_struct<index_imu_tuple<0>, Order> {
		...
		//微分をしないとき
		template <>
		class make_differentiation_struct<index_imu_tuple<>> {
			_Automatic_partial_differentiation<R(Args...), D> f;
		public:
			constexpr make_differentiation_struct(function<typename diff_type<R, D>::type(typename diff_type<Args, D>::type...)>& f) :f(f) {}
		...
		struct order_inverse<index_imu_tuple<Inv...>, FOrder, Orders...>
			:public order_inverse<index_imu_tuple<FOrder, Inv...>, Orders...> {};
		template <imsize_t... Inv>
		struct order_inverse<index_imu_tuple<Inv...>> {
			using type = index_imu_tuple<Inv...>;
		};
		//index_imu_tupleの要素を逆順にする
		template <class T>
		struct index_imu_tuple_inverse;
		template <imsize_t... Orders>
		struct index_imu_tuple_inverse<index_imu_tuple<Orders...>> {
			using type = typename order_inverse<index_imu_tuple<>, Orders...>::type;
		};
	public:
		...
		template <imsize_t... Orders>
		R get(Args... args) {
			make_differentiation_struct<typename index_imu_tuple_inverse<typename index_imu_range<0, sizeof...(Orders)>::type>::type, Orders...> diff_struct(f);
...
namespace iml {
	...
	public:
		...
		vector<R, sizeof...(Args)> operator()(Args... args) {
			return make_gradient_vector_struct<typename index_imu_range<0, sizeof...(Args)>::type>::__make_gradient_vector(grad, iml::forward<Args>(args)...);
		}
	};
