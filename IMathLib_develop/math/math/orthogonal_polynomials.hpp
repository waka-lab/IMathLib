...
namespace iml {
	...
	//ラゲール多項式(L_{n}(x))
	template <class T>
	inline constexpr auto laguerre_polynomials(imsize_t n, const T& x) {
	...
	//ラゲール陪多項式(L_{n}^{k}(x))
	template <class T>
	inline constexpr auto associated_laguerre_polynomials(imsize_t n, imsize_t k, const T& x) {
		using result_type = typename math_function_type<T>::type;
	...
	//ルジャンドル多項式(P_{k}(x))
	template <class T>
	inline constexpr auto legendre_polynomials(imsize_t k, const T& x) {
		using result_type = typename math_function_type<T>::type;
	...
	//ルジャンドル陪多項式(P_{k}^{m}(x))
	template <class T>
	inline constexpr auto associated_legendre_polynomials(imsize_t k, imsize_t m, const T& x) {
		using result_type = typename math_function_type<T>::type;
