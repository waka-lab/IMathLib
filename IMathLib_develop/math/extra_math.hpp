...
namespace iml {
	...
	inline float frexp(float x, int_t* e) {
		float buf = log10(abs(x));
		*e = static_cast<int_t>(buf);
	...
	inline double frexp(double x, int_t* e) {
		double buf = log10(abs(x));
		int_t temp = static_cast<int_t>(buf);
	...
	inline long double frexp(long double x, int_t* e) {
	...
	inline T imint_tegral(F f, const T& first, const T& last, const T& epsilon = 1E-5) {
		...
		//分割数の取得
		n = static_cast<size_t>(sqrt(sqrt(abs(pow(last - first, 5)*f_max / (epsilon * 180)))));
