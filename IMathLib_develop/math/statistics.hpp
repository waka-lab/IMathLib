	...
	namespace stats {
		...
		struct statistice_function {
			...
			static value_type __accumulate(iterator first, iterator last) {
				value_type result = addition<value_type>::identity_element();
				while (first != last) {
					result += *first;
					++first;
				}
		...
		private:
			...
			struct __median_struct {
				...
				static value_type __quick_sort_median(iterator first, iterator last, imsize_t n) {
				...
				//間接アクセス的な役割の関数
				static value_type __find(iterator first, iterator last, imsize_t n) {
			...
			struct __median_struct<bidirectional_iterator_tag> {
				...
				//クイックソートを利用した中央値探索(実際はn番目に小さい数の探索)
				static value_type __quick_sort_median(iterator first, iterator last, imsize_t n, imsize_t dis) {
					if (dis <= 1) return first[n];
					iterator i = first, j = last - 1;
					--dis;
					value_type pivot = __median3(*i, *next(i, dis / 2), *j);
					imsize_t cnt = 0;							//イテレータの変化のカウンタ
					while (true) {
						while (*i < pivot) { ++i; ++cnt; }
						while (pivot < *j) { --j; ++cnt; }
						if (dis <= cnt) break;
		...
		public:
			...
			//n番目に大きい値(可変長要素)
			template <class... Args>
			static value_type __nth_biggest(imsize_t n, const value_type& first, Args... args) {
				value_type temp[sizeof...(Args)+1] = { first,static_cast<value_type>(args)... };
				return __nth_smallest(n, temp, &temp[sizeof...(Args)+1]);
			}
			//n番目に大きい値(ポインタ)
			static value_type __nth_biggest(imsize_t n, iterator first, iterator last) {
				//偶数なら2つの平均(奇数の方が高速)
				return __median_struct<>::__find(first, last, distance(first, last) - n - 1);
			}
			//n番目に小さい値(可変長要素)
			template <class... Args>
			static value_type __nth_smallest(imsize_t n, const value_type& first, Args... args) {
				value_type temp[sizeof...(Args)+1] = { first,static_cast<value_type>(args)... };
				return __nth_smallest(n, temp, &temp[sizeof...(Args)+1]);
			}
			//n番目に小さい値(ポインタ)
			static value_type __nth_smallest(imsize_t n, iterator first, iterator last) {
				//偶数なら2つの平均(奇数の方が高速)
		...
		private:
			//最頻値テーブルを評価
			static value_type __mode_table_evaluation(value_type* table, imsize_t size) {
				//ソート済みテーブルで最も連続して同じ数が出現するものの探索
				value_type result = table[0], sum = 0, temp = 0;
				for (imsize_t i = 1; i < size; ++i, ++temp) {
		...
		//n番目に大きい値
		template <class First, class... Rest>
		inline First nth_biggest(imsize_t n, First first, Rest... rest) {
			return statistice_function<First*>::__nth_biggest(n, first, iml::forward<Rest>(rest)...);
		}
		template <class BidirectionalIterator>
		inline typename iterator_traits<BidirectionalIterator>::value_type nth_biggest(imsize_t n, BidirectionalIterator first, BidirectionalIterator last) {
			return statistice_function<BidirectionalIterator>::__nth_biggest(n, first, last);
		}
		//n番目に小さい値
		template <class First, class... Rest>
		inline First nth_smallest(imsize_t n, First first, Rest... rest) {
			return statistice_function<First*>::__nth_smallest(n, first, iml::forward<Rest>(rest)...);
		}
		template <class BidirectionalIterator>
		inline typename iterator_traits<BidirectionalIterator>::value_type nth_smallest(imsize_t n, BidirectionalIterator first, BidirectionalIterator last) {
		...
		namespace stats {
			...
			public:
				...
				type operator()(rnd::random_base<S>* handle) {
					...
					//整数部
					x_int = 0;
					for (imsize_t i = 0; i < int_beta; ++i) x_int += -log(handle->unit<type>());		//平均が1の指数分布乱数の加算
