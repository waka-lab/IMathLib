...
namespace iml {
	...
	struct _Manhattan_norm<vector<T, N>> {
		...
		static result_type __manhattan_norm(const vector<T, N>& x) {
			result_type temp = addition<T>::identity_element();
			for (imsize_t i = 0; i < N; ++i) temp += abs(x[i]);
			return temp;
		}
	};
	...
	struct _Manhattan_norm<matrix<T, M, N>> {
		...
		static result_type __manhattan_norm(const matrix<T, M, N>& x, imsize_t n) {
			result_type temp = addition<T>::identity_element();
			for (imsize_t i = 0; i < dimension<M, N>::value; ++i) temp += abs(x[0][i]);
			return temp;
		}
	};
	template <class T>
	...
	struct _Euclid_norm<matrix<T, M, N>> {
		...
		static result_type __euclid_norm(const matrix<T, M, N>& x, imsize_t n) {
			result_type temp = addition<T>::identity_element();
			for (imsize_t i = 0; i < dimension<M, N>::value; ++i) temp += abs(x[0][i])*abs(x[0][i]);
			return sqrt(temp);
		}
	};
	template <class T>
	...
	struct _Chebyshev_norm<vector<T, N>> {
		...
		static result_type __chebyshev_norm(const vector<T, N>& x) {
			result_type temp = addition<T>::identity_element();
			for (imsize_t i = 0; i < N; ++i)  if (abs(x[i]) > temp) tenp = abs(x[i]);
			return temp;
		}
	};
	template <class T, imsize_t M, imsize_t N>
	struct _Chebyshev_norm<matrix<T, M, N>> {
		...
		static result_type __chebyshev_norm(const matrix<T, M, N>& x, imsize_t n) {
			result_type temp = addition<T>::identity_element();
			for (imsize_t i = 0; i < dimension<M, N>::value; ++i) if (abs(x[0][i]) > temp) tenp = abs(x[0][i]);
			return temp;
		}
	};
	template <class T>
	...
	struct _Lp_norm {
		...
		static constexpr result_type __lp_norm(const T& x, imsize_t n) {
			return abs(x);
		}
	};
	template <class T, imsize_t N>
	struct _Lp_norm<vector<T, N>> {
		...
		static result_type __lp_norm(const vector<T, N>& x, imsize_t n) {
			result_type temp = addition<T>::identity_element();
			for (imsize_t i = 0; i < N; ++i) temp += pow(abs(x[i]), n);
	...
	struct _Lp_norm<matrix<T, M, N>> {
		...
		static result_type __lp_norm(const matrix<T, M, N>& x, imsize_t n) {
			result_type temp = addition<T>::identity_element();
			for (imsize_t i = 0; i < dimension<M, N>::value; ++i) temp += pow(abs(x[0][i]), n);
			return pow(temp, result_type(1) / n);
		}
	};
	template <class T>
	inline constexpr auto lp_norm(const T& x, imsize_t n) { return _Lp_norm<T>::__lp_norm(x, n); }
