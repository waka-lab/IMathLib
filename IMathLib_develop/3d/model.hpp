#ifndef IMATHLIB_3D_MODEL_HPP
#define IMATHLIB_3D_MODEL_HPP
...
namespace iml {
	namespace m3d {
		...
		// 型定義
		using byte = iml::uint8_t;
		using word = iml::uint16_t;
		using dword = iml::int32_t;
		...
		using mat_array_data = pair<shared_ptr<mat_data>, dword>;
		// 頂点配列データ
		template <class T>
		struct vertex_data {
		...
		template <class T>
		using vertex_array_data = pair<shared_ptr<vertex_data<T>>, word>;
		...
		using vertex_index = pair<shared_ptr<word>, dword>;
		// マテリアル情報
		template <class T>
