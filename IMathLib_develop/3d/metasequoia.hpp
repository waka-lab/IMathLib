#ifndef IMATHLIB_INTERFACE_3D_METASEQUOIA_HPP
#define IMATHLIB_INTERFACE_3D_METASEQUOIA_HPP
	...
	namespace m3d {
		...
		inline void _Make_polygon_data(inner_object<T>* model, object_data<T>* obj, mat_array_data& mat) {
			...
			for (size_t i = 0; i < model->mat.size(); ++i) {
				...
				material->vtx_index.first.reset(new word[material->vtx_index.second], dealloc_ident<dealloc::array>());
			...
			void read_object(L& line_obj, object_data<T>* obj, const T& scale) {
				...
				while (str = line_obj.get_line(), string_find(str, "}", 1) == nullptr) {
					...
					// vertex
						...
						obj->vertex_info.first.reset(new vertex_data<T>[obj->vertex_info.second], dealloc_ident<dealloc::array>());
			...
			void mqo_load_1_0(L& line_obj, const T& scale) {
				...
				for (auto itr =(obj.push_back(object_data<T>()),obj.begin()); str = line_obj.get_line();) {
					...
					// Material(1回のみ)
						...
						mat.first.reset(new mat_data[mat.second], dealloc_ident<dealloc::array>());
				...
				for (size_t i = 0; i != mat.second; ++i) {
					if (mat.first[i].tex_name[0] != 0) {
						...
						std::string temp = (string_find(mat.first[i].tex_name, ":", 1) == nullptr)
