...
namespace iml {
	namespace i_f {
		...
		class ttf {
			TTF_Font*		font_ptr;
			size_t		point_size;			//フォントのポイントサイズ
			size_t		line_width;			//改行幅
			size_t		font_height;		//フォントの高さ
		public:
		...
		class string_texture {
			...
			//文字列の横幅を取得
			//UTF-16
			size_t get_text_width(const ttf& font, wchar_t* str) {
				int temp;
				TTF_SizeUNICODE(font(), reinterpret_cast<Uint16*>(str), &temp, nullptr);
				return temp;
			}
			//UTF-8
			size_t get_text_width(const ttf& font, char* str) {
				int temp;
		...
		public:
			...
			template <class CharT>
			string_texture(const ttf& font, const CharT* str, const color_32bit &color, size_t width = -1) {
			...
			template <class CharT>
			void push_back(const ttf& font, const CharT* str, const color_32bit &color, size_t width = -1) {
				...
				while (1) {
					...
					if (width == static_cast<size_t>(-1)) str_array.push_back(new unit_string_texture(font, p, color, true));
					else {
						size_t i, j = 0;
						for (i = 0; p[i] != '\0'; ++i) {
			...
			unit_string_texture* operator[](size_t n) {
				return str_array[n];
			}
			unit_string_texture* operator[](size_t n) const {
				return str_array[n];
			}
			size_t size() const { return str_array.size(); }
		};
		...
		template <class CharT>
		inline void draw_string_texture2(double x, double y, const ttf& font, const CharT* str, const color_32bit &color, size_t width = -1) {
