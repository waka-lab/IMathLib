...
namespace iml {
	namespace i_f {
		...
		public:
			...
			//押している
			bool push(int_t code) const { return m_now[code]; }
			//押した瞬間
			bool trigger(int_t code) const { return (m_now[code] && !m_last[code]); }
			//離した瞬間
			bool release(int_t code) const { return (!m_now[code] && m_last[code]); }
		};
		...
		class mouse_input : public singleton<mouse_input> {
			...
			vector2<int_t>	m_now;		//現在の座標
			vector2<int_t>	m_last;		//前回の座標
		...
		public:
			...
			//現在のマウス座標
			const vector2<int_t>& now_mouse() { return m_now; }
			//前回からの相対マウス座標(移動距離)
			vector2<int_t> move_mouse() { return m_now - m_last; }
			//クリックしている
