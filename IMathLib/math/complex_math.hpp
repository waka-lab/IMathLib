...
namespace iml {
	...
	private:
		//冪乗の分岐のための構造体
		//一般
		template <class S, bool flag1 = is_same<S, int_t>::value, bool flag2 = is_same<S, size_t>::value>
		struct __pow_struct {
		...
		struct __pow_struct<S, true, false> {
			static type __pow(const type& x, int_t y) {
				if (y == 0) return 1;
				if (y == 1) return x;
				if (y == 2) return x*x;
				if (y < 0) {
		...
		struct __pow_struct<S, false, true> {
			static type __pow(const type& x, size_t y) {
				if (y == 0) return 1;
				if (y == 1) return x;
				if (y == 2) return x*x;
				if ((y & 1) == 0) return __pow(__pow(x, y >> 1), 2);
